...
This installs the skill globally for Codex. For another supported agent, replace `codex` after `-a`, or omit `-a codex` to let the CLI choose the available agent integrations. The CLI runs directly through `npx`; no separate global npm installation is required.
...
[`references/anti-adhesion-hydrogel-example.en.md`](references/anti-adhesion-hydrogel-example.en.md) is the English worked reference for postoperative abdominal/peritoneal adhesion prevention. The [Chinese worked reference](references/anti-adhesion-hydrogel-example.md) is also available.

These references demonstrate literature curation, journal metadata, causal evidence tracing, first-principles comparison dimensions, gap analysis, and non-combinatorial Idea Cards. They are structural examples, not substitutes for re-verifying sources in a new project.
